...
class ViewSamplerEvaluation(ViewSampler[ViewSamplerEvaluationCfg]):
    ...
    def __init__(
        ...
        if 'scannetpp' in str(index_path):
            index_path = Path(str(index_path).replace('scannetpp.json', f'scannetpp_{cfg.num_context_views}views.json'))
            if stage == 'test_fvs':
                index_path = Path(str(index_path).replace('.json', '_fvs.json'))
        elif 'scannet' in str(index_path):
            index_path = Path(str(index_path).replace('index_scannet.json', f'scannet_{cfg.domain}.json'))                
