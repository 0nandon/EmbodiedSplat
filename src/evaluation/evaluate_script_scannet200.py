...
from database.class_labels import SCANNET200_CLASS_LABELS, SCANNET200_IGNORE
...
SCANNET_LABELS_200_FORMAT = [
    "a {} in a photo.".format(cls) for cls in SCANNET200_CLASS_LABELS if cls not in SCANNET200_IGNORE
]
...
def process_gt(raw_gt, labels_info):
    ...
    for idx, label in labels_info.items():
        if label['name'] in SCANNET200_CLASS_LABELS and label['name'] not in SCANNET200_IGNORE:
            gt[raw_gt == idx] = cnt
            cnt += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--labels-yaml", type=str,
                        default="src/evaluation/database/scannet200_label_database.yaml")
    ...
    parser.add_argument("--dataset", type=str, default="scannet200")
    args = parser.parse_args()

    text_feat = build_text_embedding(SCANNET_LABELS_200_FORMAT, mode=args.text_mode)
    ...
    for scene in tqdm(os.listdir(PRED_PATH)):
        ...
        pcd = np.load("dataset/scannet/test/" + scene + "/" + scene[5:] + "_scannet200.npy")
