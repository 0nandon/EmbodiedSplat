...
class GRU2D_naive_Wweights(nn.Module):
    def __init__(self, concat_depth=False):
        ...
        self.weight_channel = weights_dim
        ...
        self.mlp_z = nn.Sequential(nn.Linear(hidden_channel + input_channel + 2*weights_dim + concat_depth*2, hidden_channel),
                                   nn.ReLU(),
                                   nn.Linear(hidden_channel, hidden_channel))
        self.mlp_r = nn.Sequential(nn.Linear(hidden_channel + input_channel + 2*weights_dim + concat_depth*2, hidden_channel),
                                   nn.ReLU(),
                                   nn.Linear(hidden_channel, hidden_channel))
    ...
    def forward(self, input_feat, hidden_feat, input_weights_emb, hidden_weights_emb):
        ...
        update_feat = torch.cat((r * hidden_feat[...,:self.channel], input_feat_1[...,:self.channel+self.weight_channel]), dim=-1)
        q = torch.tanh(self.mlp_n(update_feat))
        output = (1 - z) * hidden_feat[...,:self.channel] + z * q
        return output