...
def batch_features(features, limit=3):
    ...
    full_indices = torch.arange(n_views, device=features[0].device)[None].repeat(n_views,1)
    cur_indices = torch.arange(n_views, device=features[0].device)
    judge = 0
    # print(f'n_views: {n_views}, limit: {limit}')
    if n_views > limit:
        slide_mask = torch.zeros((n_views, n_views), dtype=torch.bool, device=features[0].device)
        for i in range(n_views):
            if i < limit // 2:
                start = 0
                end = limit
            elif i >= n_views - limit // 2:
                start = n_views - limit
                end = n_views
            else:
                start = i - (limit-1) // 2
        ...
        src_indices = full_indices[(~(full_indices == cur_indices[:,None]))*slide_mask].view(n_views,limit-1)
        judge = 1
        
    # else:
    #     src_indices = full_indices[~(full_indices == cur_indices[:,None])].view(n_views,n_views-1)
    #     judge = 0
    # print(f'n_views: {n_views}, features[0].shape: {features[0].shape}, limit: {limit}')
    for i in range(n_views):
        x = features.copy()
        q.append(x[i] if judge else x.pop(i))  # [B, C, H, W] or [B, H*W, C]
    ...
    if judge:
        if kv.dim() == 4:
            h, w = kv.shape[-2:]
            src_indices_expanded = src_indices.unsqueeze(-1).unsqueeze(-1)
            src_indices_expanded = src_indices_expanded.expand(-1, -1, h, w)

            # Gathering the data
            kv = torch.gather(kv, 1, src_indices_expanded)
        else:
            c, h, w = kv.shape[-3:]
            src_indices_expanded = src_indices.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
            src_indices_expanded = src_indices_expanded.expand(-1, -1, c, h, w)
...
class MultiViewFeatureTransformer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        concat0, concat1 = batch_features(multi_view_features, limit=self.limit)
        concat0 = concat0.reshape(num_views * b, c, -1).permute(
            0, 2, 1
        )  # [N*B, H*W, C]
        concat1 = concat1.reshape(num_views * b, concat1.shape[1], c, -1).permute(
            0, 1, 3, 2
        )  # [N*B, N-1, H*W, C]
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i < len(self.layers) - 1:
                ...
                concat0, concat1 = batch_features(features, limit=self.limit)
