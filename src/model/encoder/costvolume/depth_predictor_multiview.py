...
def prepare_feat_proj_data_lists(
    features, intrinsics, extrinsics, near, far, num_samples,limit=3
):
    # prepare features
...
class DepthPredictorMultiView(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # print('depth_predictor_num_views:', num_views)
    ...
    def forward(
        ...
        # print('self.num_depth_candidates:', self.num_depth_candidates)
        ...
        if self.wo_cost_volume:
            ...
            # print('len(feat_comb_lists):', len(feat_comb_lists))
            for feat10, pose_curr in zip(feat_comb_lists[1:], pose_curr_lists):
                # sample feat01 from feat10 via camera projection
                # print(f'feat01.shape: {feat01.shape}, feat10.shape: {feat10.shape}, intr_curr.shape: {intr_curr.shape}, pose_curr.shape: {pose_curr.shape}, disp_candi_curr.shape: {disp_candi_curr.shape}')
                raw_correlation_in = []
                if len(feat10) < 10:
                    ...
                    for i in range(round(len(feat10)/10.)):
                        feat01_warped = warp_with_pose_depth_candidates(
                            feat10[i*10:(i+1)*10],
                            intr_curr[i*10:(i+1)*10],
                            pose_curr[i*10:(i+1)*10],
                            1.0 / disp_candi_curr[i*10:(i+1)*10].repeat([1, 1, *feat10.shape[-2:]]),
                            warp_padding_mode="zeros",
                        )  # [B, C, D, H, W]
                        # calculate similarity
                        raw_correlation_in.append((feat01[i*10:(i+1)*10].unsqueeze(2) * feat01_warped).sum(
                            1
                        ) / (
                            c**0.5
                        ))  # [vB, D, H, W]
                    raw_correlation_in = torch.cat(raw_correlation_in, dim=0)
                # print('wraped ratio:', (feat01_warped!=0).sum()/feat01_warped.numel())
