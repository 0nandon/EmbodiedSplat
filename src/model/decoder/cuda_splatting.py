...
from diff_gaussian_rasterization_lang import (
    GaussianRasterizationSettings as GaussianRasterizationSettingsLang,
    GaussianRasterizer as GaussianRasterizerLang,
)
...
def render_cuda(
    ...
    gaussian_features: Float[Tensor, "batch gaussian 64"],
    ...
    for i in range(b):
        ...
        if render_feature:
            settings_lang = GaussianRasterizationSettingsLang(
                image_height=h,
                image_width=w,
                tanfovx=tan_fov_x[i].item(),
                tanfovy=tan_fov_y[i].item(),
            ...
            rasterizer_lang = GaussianRasterizerLang(raster_settings=settings_lang)
    ...
    return torch.stack(all_images), torch.stack(all_depths), None, torch.stack(all_feats) if all_feats[0] is not None else all_feats
