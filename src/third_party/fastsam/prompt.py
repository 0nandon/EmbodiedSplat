...
class FastSAMPrompt:
    ...
    def _format_results(self, result, filter=0, sort=False):
        annotations = []
        n = len(result.masks.data)

        if sort:
            masks = result.masks.data
            mask_size = masks.sum(-1).sum(-1)
            sort_idx = mask_size.argsort(dim=-1, descending=True)
        ...
        return annotations, result.masks.data
    ...
    def _crop_image_torch(self, format_results, img_idx=None, resize=None):
        annotations = format_results

        cropped_imgs = []
        for _, mask in enumerate(annotations):
            bbox = self._get_bbox_from_mask(mask['segmentation'])
            cropped_img = self.img[img_idx, :, bbox[1]:bbox[3], bbox[0]:bbox[2]].unsqueeze(0)
