...
class MSGradientLoss(nn.Module):
    ...
    def forward(self, depth_gt: Tensor, depth_pred: Tensor) -> Tensor:
        ...
        for depth_pred_down, depth_gtn_down in zip(depth_pred_pyr, depth_gtn_pyr):
            ...
            mask = ((depth_gtn_down > 1e-3) * (depth_gtn_down < 10)).type(torch.float32)

            kernel_size = 2
            kernel = torch.ones(kernel_size, kernel_size, device=mask.device)
            mask = (1 - kornia.morphology.dilation(1-mask, kernel)).type(torch.bool).unsqueeze(2)

            depth_pred_grad = kornia.filters.spatial_gradient(
                                    depth_pred_down).masked_select(mask & mask_down_b)

            grad_error = torch.abs(depth_pred_grad - 
                                    depth_gtn_grad.masked_select(mask & mask_down_b))
